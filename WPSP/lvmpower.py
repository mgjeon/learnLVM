...
# @Date: 2021-07-19
# You should test def on(), off(), statusdictionary(),
# + apply to lvmnps actor command "onoff.py"
...
class LVMPowerSwitch(object):
    ...
    async def on(self, outlet_number:int=0, name:str='Outlet 0'):
        """Turn on power to an outlet"""

        outlets = await self.statusdictionary()
        len = range(0, 7)
    ...
    async def onall(self):
        ...
        return await self.statusdictionary()


    async def off(self, outlet_number:int=0, name:str='Outlet 0'):
    ...
    async def getstatus(self):
        ...
        each item will contatin 2 items hostname and state"""
        i = 1
        data = {}
